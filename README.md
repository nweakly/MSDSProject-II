...
- The loss function is an important training indicator, but a lower loss does not necessarily guarantee a better model.  When training on a dataset of images scraped from Google image search, I reached the lowest moving average loss of all my model - 0.5441. However, the model had a very low recall. Training on a dataset of still images extracted from the Ring videos resulted in a moving average loss of 0.6824, however, the model had a high false positive rate. It was incorrectly identifying crowbars where they were previously present in training images, clearly picking up on some other features.  When training on a more balanced set, the moving average loss was fluctuating between 0.9 and 1.05 and not decreasing any further.  This model turned out to provide better detection results. 
...
- The loss function is important but the lower loss does not always guarantee a better model.  The lowest moving average loss of 0.5441 I received when training only on images collected from the internet, the second best was about 0.6825  received on a set of still images extracted from the Ring videos. A more balanced training set combining images from all sources and generating better prediction results had moving average loss function fluctuating between 0.9 and 1.01  that was not decreasing any further with additional training.
